...
export default function NavBar() {
  ...
  const navigation = [
    ...
    { name: 'En Vivo', href: '/en-vivo', highlight: true },
  ];
            ...
            <div className="hidden sm:ml-6 sm:flex sm:space-x-8">
              {navigation.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
                  className={`inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium border-transparent ${
                    item.highlight
                      ? 'text-red-600 hover:border-red-700 hover:text-red-800 focus:ring-red-500 animate-pulse motion-reduce:animate-none'
                      : 'text-gray-500 hover:border-blue-500 hover:text-gray-700 focus:ring-blue-500'
            ...
            {navigation.map((item) => (
              <Link
                ...
                className={`block pl-3 pr-4 py-2 border-l-4 text-base font-medium border-transparent ${
                  item.highlight
                    ? 'text-red-600 hover:bg-red-50 hover:border-red-500 hover:text-red-700'
                    : 'text-gray-500 hover:bg-blue-50 hover:border-blue-500 hover:text-blue-700'
                }`}
