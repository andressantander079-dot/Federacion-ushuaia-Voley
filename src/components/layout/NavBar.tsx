              ...
              {NAV_LINKS.map(({ href, label, special }) => {
                ...
                return (
                  <Link
                    ...
                    className={`inline-flex items-center px-1 pt-1 border-b-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors ${
                      special
                        ? `text-red-600 hover:border-red-700 hover:text-red-800 focus:ring-red-500 ${isActive ? 'border-red-500' : 'border-transparent'} animate-pulse motion-reduce:animate-none`
                        : `text-gray-500 hover:border-blue-500 hover:text-gray-700 focus:ring-blue-500 ${isActive ? 'border-blue-500 text-gray-900' : 'border-transparent'}`
                    }`}
                  >
                    {label}
                  </Link>
